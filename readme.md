...
It detects when CC is working, idle, prompting, stalled, context-overflowed, crashed, or rate-limited, and takes the appropriate action. State is persisted to JSON between tick fires — Clawbot has no memory across fires except the state file.
...
│   ├── clawbot-watchdog.sh    re-bootstraps the tick LaunchAgent if it's dead
...
<your-bmad-project>/
├── memory/
│   ├── bmad-dev-state.json    Clawbot's "brain" — read every tick fire
...
- macOS (the notifier, `date -j -f` parsing, and the launchd scheduler are macOS-specific)
- `claude` CLI installed and logged in (Claude Max plan recommended)
- `tmux`, `jq`, `yq`, `flock`, `osascript`, `launchctl`
...
| Command | Effect |
|---|---|
| `status` | Prints current story/step/queue, tick health, epic-review state, rate-limit info if active |
...
macOS Notification Center banners fire for everything except `[WORKING]` (which would fire every tick — spam).
...
2. **Sleep** (`bin/clawbot-tick.sh`, shell-level): while `status == "rate-limited"` and `now < resumeAt`, every tick fire logs `CRON_SKIP | reason:rate-limited | remaining_min:N` and exits — no `claude -p` invocation, so the cooldown costs zero headless tokens.
...
```jsonc
{
  ...
  "quarantinedStories": [...],
  "modelStrategy": { ... },
  "cronHealth": {                // legacy name — tracks the launchd tick's heartbeat
    "lastCronFire": "...",      // watchdog uses this — if >10m stale, re-bootstrap the tick LaunchAgent
    "consecutiveFires": 42,
    "cronStatus": "healthy"
  },
